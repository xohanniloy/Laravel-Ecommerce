    ...
    form.post('/products', {
        onSuccess: () => {
            if (page.props.flash?.status == true) {
                toaster.success(page.props.flash?.message || 'Product added successfully');
                router.get("/products");
            } else {
                toaster.error(page.props.flash?.message || "Product add failed. Please check your credentials.");
            }
        }
    });
}
